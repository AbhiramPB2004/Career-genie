...
origins = [
    ...
    "http://localhost:5173",
    "http://localhost:5173",
    "http://localhost:3000"  # Add the origin causing the CORS error
...
def chat_completions(prompt:ChatPrompt):
    # Call the OpenAI chat completion API here
    # Use the prompt parameter as the input to the API
    # Return the response from the API as the result
    messages = [
    {"role":"system","content":"you are an chatbot who can help with queries related to career guidance. your job is to suggest career and help them out with their queries"}
    ]
...
def get_root():
    ...
    # loadedModel.predict('input_data')
